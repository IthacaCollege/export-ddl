...
BEGIN
  LOOP
    ...
    if obj.object_type='PACKAGE' then
      ...
      ITHACA.ITHACA_DDL_EXPORT.P_OBJECT_DDL( obj.owner, obj.object_type, obj.object_name);
    end if;
  END LOOP;
END P_OBJECT_SCRIPT;
...
BEGIN
  ...
  SYS.UTL_FILE.PUT_LINE(DDL_FILE,'--------------------------------------------------------');
  SYS.UTL_FILE.PUT_LINE(DDL_FILE,'');
  SYS.UTL_FILE.PUT_LINE(DDL_FILE,DBMS_METADATA.GET_DDL(P_OBJECT_TYPE,P_OBJECT_NAME,P_SCHEMA));
  SYS.UTL_FILE.PUT_LINE(DDL_FILE,'');
  FOR GRT IN (
